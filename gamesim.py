...
class trumpPlayer(Player): 
    ...
    def makeMove(self, gs):
        ...
        if (len(gs.table) == 0): ##if no card is on the table yet 
            ...
            #selects cards using the trump suit choosing to play the highest card first
            chosen = gs.trumpSuit
            if (isinstance(self.handsDic.get(chosen),list) and len(self.handsDic.get(chosen)) != 0):
                if ("J" in self.handsDic.get(chosen)): 
                    hand = ("J",chosen,30)
                    gs.table.append(hand)
                    self.handsDic.get(chosen).remove("J")
                    self.hand.remove(hand)
                elif ("9" in self.handsDic.get(chosen)): 
                    hand = ("9",chosen,20)
                    gs.table.append(hand)
                    self.handsDic.get(chosen).remove("9")
                    self.hand.remove(hand)
                elif ("A" in self.handsDic.get(chosen)): 
                    hand = ("A",chosen,11)
                    gs.table.append(hand)
                    self.handsDic.get(chosen).remove("A")
                    self.hand.remove(hand)
                elif ("10" in self.handsDic.get(chosen)): 
                    hand = ("10",chosen,10)
                    gs.table.append(hand)
                    self.handsDic.get(chosen).remove("10")
                    self.hand.remove(hand)
                elif ("Q" in self.handsDic.get(chosen)): 
                    hand = ("Q",chosen,2)
                    gs.table.append(hand)
                    self.handsDic.get(chosen).remove("Q")
                    self.hand.remove(hand)
                elif ("K" in self.handsDic.get(chosen)): 
                    hand = ("K",chosen,3)
                    gs.table.append(hand)
                    self.handsDic.get(chosen).remove("K")
                    self.hand.remove(hand)
                elif ("7" in self.handsDic.get(chosen)): 
                    hand = ("7",chosen,0)
                    gs.table.append(hand)
                    self.handsDic.get(chosen).remove("7")
                    self.hand.remove(hand)
                elif ("8" in self.handsDic.get(chosen)): 
                    hand = ("8",chosen,0)
                    gs.table.append(hand)
                    self.handsDic.get(chosen).remove("8")
                    self.hand.remove(hand)
                    
            # if no trump card is available 
            else: 
                selected = False 
                allSuits = ["H", "D", "S", "C"]
                while (selected == False): 
                    randomSuit = random.choice(allSuits)
                    if (randomSuit != chosen):
                        if (isinstance(self.handsDic.get(randomSuit),list) and len(self.handsDic.get(randomSuit)) != 0):
                            selected = True 
                            chosen = randomSuit
                            if ("J" in self.handsDic.get(chosen)): 
                                hand = ("J",chosen,30)
        ...
        # if there is card on the table 
            ...
            if (self.handsDic.get(suits) == 0): #if the player does not have that suit in their hand 
                if (not gs.trumpIsOpen):     
                    gs.trumpIsOpen = True   ## trump card is now open 
                    chosen = gs.trumpSuit
                    ...
                    while (selected == False): 
                        ...
                        if (randomSuit != chosen):
                            ...
                            if ("J" in self.handsDic.get(chosen)): 
                                ...
                                self.handsDic.get(chosen).remove("8")
                                self.hand.remove(hand)
            # if the player does have the suit, then play the highest card in that suit in the hand 
            else:
                if ("J" in self.handsDic.get(chosen)): 
                    hand = ("J",chosen,30)
                    gs.table.append(hand)
...
class MajTrumpPlayer(trumpPlayer): 
    def __init__(self):
        super().__init__()
...
ciGames = [[MajTrumpPlayer(), MajBigPlayer(), MajTrumpPlayer(), MajBigPlayer()], [MajTrumpPlayer(), ValueSmallPlayer(), MajTrumpPlayer(), ValueSmallPlayer()], 
[MajTrumpPlayer(), TopBigPlayer(), MajTrumpPlayer(), TopBigPlayer()], 
[MajTrumpPlayer(), TopSmallPlayer(), MajTrumpPlayer(), TopSmallPlayer()], [MajTrumpPlayer(), MajSmallPlayer(), MajTrumpPlayer(), MajSmallPlayer()]]
expected_array = []
...
for game in ciGames:
    ...
    for i in range(10):
        gm = GameManager(game)
        gm.runGames(1000)
        estimate = gm.expectedOutcome()
        estimates.append(estimate)

    min_estimate = min(estimates)
    max_estimate = max(estimates)

    print(game[0].__class__.__name__ + " vs. " +  game[1].__class__.__name__ + ": ")
    print ("[" + str(min_estimate) + ", " + str(max_estimate) + " ]")
...
gm = GameManager([MajTrumpPlayer(), MajSmallPlayer(), MajTrumpPlayer(), MajSmallPlayer()])
gm.runGames(N_GAMES)
expected_outcome = gm.expectedOutcome()
...
for i in range(N_EXPERIMENTS):
    ...
    for g in range(N_GAMES):
        gm.runGame()
    transactions = gm.computeTransactions()
